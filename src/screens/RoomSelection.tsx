...
const RoomSelection = ({ state, setState }: Props) => {
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <div className="flex-1 p-6 flex flex-col">
        ...
        <div className="flex-1 bg-white rounded-2xl shadow-lg p-4 mb-6">
          <div className="relative w-full h-96 bg-gray-100 rounded-xl border-2 border-gray-300">
            {/* SVG Floor Plan */}
          ...
          <div className="space-y-3">
            {rooms.map((room) => {
              ...
              return (
                <label
                  key={room.id}
                  className={`p-4 rounded-xl font-semibold cursor-pointer block ${
                    isSelected
                ...
                >
                  <div className="flex items-center">
                    ...
                    />
                    <span className={`text-lg ${isSelected ? 'text-blue-900 font-bold' : 'text-gray-800'}`}>
                      {room.name}
                    </span>
                  </div>
                </label>
        ...
        >
          Confirm Rooms ({selectedRooms.length})
        </button>
      </div>
    </div>
  )
